...
void com_tcp_recv_task(void *event_group)
{
    ...
    WAIT_AP_START: while (1)    // Level 0
    {
        ...
        RECREATE_SOCKET: while (1)  // Level 1
        {
            ...
            ESP_LOGI(IAWARE_NETWORK, "Recv. conns: Try to create a socket with free heap = %d bytes.", heap_caps_get_free_size(MALLOC_CAP_8BIT));            
            ...
            WAIT_FOR_A_CLIENT: while (1)    // Level 2
            {
                ...
                ESP_LOGI(IAWARE_NETWORK, "Recv. conns: Wait for a client with free heap = %d bytes.", heap_caps_get_free_size(MALLOC_CAP_8BIT));                            
...
void com_tcp_send_task(void *event_group)
{
    ...
    WAIT_AP_START: while (1) // Level 0
    {
        ...
        RECREATE_SOCKET: while (1) // Level 1
        {
            ESP_LOGI(IAWARE_NETWORK, "Send conns: before create a socket, free heap = %d bytes.", heap_caps_get_free_size(MALLOC_CAP_8BIT));
            ...
            WAIT_FOR_A_CLIENT: while (1) // Level 2
            {
                ESP_LOGI(IAWARE_NETWORK, "Send conns: Wait for a client with free heap = %d bytes.", heap_caps_get_free_size(MALLOC_CAP_8BIT));

                // Wait for a new client to connect. This corresponds to socket.socket.connect.
                if ((cs = accept(s, NULL, NULL)) < 0)
                {
                    ESP_LOGW(IAWARE_NETWORK, "Send conns: Failed to allocate cs (%s, %d).", strerror(errno), errno);                    
