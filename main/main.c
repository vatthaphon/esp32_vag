...
struct buff_node *run_dsp_buff_node_ptr = NULL;
struct buff_node *run_dsp_buff_node_pre_ptr = NULL;
...
{   
    ...
    // {
    // ESP_LOGW(IAWARE_CORE, "Sample data: Too high sampling frequency by %" PRId64 "ms.", (cur_time - pre_time));   
    // }
    ...
    esp_log_level_set(IAWARE_DSP, ESP_LOG_LEVEL_IAWARE_DSP);
    ...
    // The task of sampling input data uses ADS1256. Therefore, the callback of the hardware timer always runs in Core 0.
    init_sampling_data_ads1256_task();    

    start_sampling_data_ads1256();    
}
...
static esp_err_t event_handler(void *ctx, system_event_t *event)
{
    switch(event->event_id) 
    {
        ...
        default:
            ESP_LOGE(IAWARE_EVENT, "Unrecognized Event ID: %d.", event->event_id);
...
static void init_wifi_in_ap(void)
{
    ...
    getSSID(&(mac[0]), &(iaware_device_name[0])); // Get the SSID generated for our device.

    ESP_LOGI(IAWARE_NETWORK, "Name of AP is %s.", iaware_device_name);    
    ...
    strcpy((char *)ap_config.ap.ssid, (char *)iaware_device_name);
    ...
    if (err != ESP_OK) 
    {
        ESP_LOGE(IAWARE_NETWORK, "Error (%s) cannot change Wifi Tx power!", esp_err_to_name(err));
    }     
    else
    {
        ESP_LOGI(IAWARE_NETWORK, "Change Wifi Tx power SUCCESS.");
    }
}
...
static void iaware_init(void)
{
    ...
    // NVS accessing.
#ifdef READ_SAMPLING_FREQUENCY_FROM_FLASH
    ESP_LOGI(IAWARE_CORE, "READ_SAMPLING_FREQUENCY_FROM_FLASH");

    nvs_read_sampling_data_fs();
#else
    ESP_LOGI(IAWARE_CORE, "Use default sampling frequenycy %d Hz.", SAMPLING_DATA_FS);

    sampling_data_fs = SAMPLING_DATA_FS;
#endif

#ifdef WRITE_SAMPLING_FREQUENCY_FROM_FLASH    
    ESP_LOGI(IAWARE_CORE, "WRITE_SAMPLING_FREQUENCY_FROM_FLASH");
